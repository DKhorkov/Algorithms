...
O(n * log(n)), а также имеет маленький постоянный коэффициент. Таким образом. в большистве сслучаев быстрая сортировка
будет более предпочтительной пиромидальной сортировке.
"""
...
class QuickSort:
    ...
    def _get_pivot_index(self, low_index: int, high_index: int) -> int:
        ...
        Возвращает опорный элемент для рекурсивного вызова сортировки.
        """
        ...
        """
        Также итерацию начинаем с нижнего цикла, чтобы не затрагивать рекурсивно уже отсортированные элементы массива.
        
        Поскольку уже работаем с индексом, то последний элемент не будет испоьзоваться в цикле, 
