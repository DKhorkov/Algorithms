...
выполняются за асимптоматические O(h), где h - высота дерева,
а обход всего дерева за O(n), где n - количество узлов в дереве.

В среднем случае, когда дерево будет сбалансированным (глубина левой и правой ветвей будет одинаковой), то
асимптоматическая скорость будет O(log(n)), где n - количество узлов в дереве.

Однако в худшем случае, когда каждый новый узел становится, например, правым потомком, то наше дерево становится
связным списком, а опреации над ним имеют асимптоматическую скорость O(n), где n - количество узлов в дереве.
"""
